...
test("simpleCart.toCurrency() function works", function() {
    ...
    deepEqual(simpleCart.toCurrency(number, {
        delimiter: " "
    }), "&#36;2 234.23", "Changing Delimiter");

    deepEqual(simpleCart.toCurrency(number, {
        delimiter: "&thinsp;"
    }), "&#36;2&thinsp;234.23", "Multi Character Delimiter");

    deepEqual(simpleCart.toCurrency(number, {
        decimal: ","
    }), "&#36;2,234,23", "Changing decimal delimiter");

    deepEqual(simpleCart.toCurrency(number, {
        symbol: "!"
    }), "!2,234.23", "Changing currency symbol");

    deepEqual(simpleCart.toCurrency(number, {
        accuracy: 1
    }), "&#36;2,234.2", "Changing decimal accuracy");

    deepEqual(simpleCart.toCurrency(number, {
        after: true
    }), "2,234.23&#36;", "Changing symbol location");

    deepEqual(simpleCart.toCurrency(number, {
        symbol: "",
        accuracy: 0,
        delimiter: ""
    }), "2234", "Long hand toInt string");
...
test("simpleCart.find() function works", function() {
    ...
    var bob = simpleCart.add({
        ...
        orange_items = simpleCart.find({
            color: 'orange'
        }),
        expensive = simpleCart.find({
            price: '>=4'
        }),
        small = simpleCart.find({
            size: '<5'
        }),
        bob_search = simpleCart.find({
            name: "bob"
        }),
        blue_and_big = simpleCart.find({
            color: 'blue',
            size: '>4'
        });
...
test("basic outlets work", function() {
    ...
    document.getElementById('test_id').innerHTML = simpleCart.quantity();
    deepEqual(document.getElementById('simpleCart_quantity').innerHTML,
              document.getElementById('test_id').innerHTML, "quantity outlet works");

    document.getElementById('test_id').innerHTML = simpleCart.toCurrency(simpleCart.total());
    deepEqual(document.getElementById('simpleCart_total').innerHTML,
              document.getElementById('test_id').innerHTML, "total outlet works");

    document.getElementById('test_id').innerHTML = simpleCart.taxRate().toFixed();
    deepEqual(document.getElementById('simpleCart_taxRate').innerHTML,
              document.getElementById('test_id').innerHTML, "taxRate outlet works");

    document.getElementById('test_id').innerHTML = simpleCart.toCurrency(simpleCart.tax());
    deepEqual(document.getElementById('simpleCart_tax').innerHTML,
              document.getElementById('test_id').innerHTML, "tax outlet works");

    document.getElementById('test_id').innerHTML = simpleCart.toCurrency(simpleCart.shipping());
    deepEqual(document.getElementById('simpleCart_shipping').innerHTML,
              document.getElementById('test_id').innerHTML, "shipping outlet works");

    document.getElementById('test_id').innerHTML = simpleCart.toCurrency(simpleCart.grandTotal());
    deepEqual(document.getElementById('simpleCart_grandTotal').innerHTML,
              document.getElementById('test_id').innerHTML, "grand total outlet works");

});


// just in case we refresh ;)
simpleCart.empty();
simpleCart.add({
    name: "Cool T-shirt",
    price: 25,
